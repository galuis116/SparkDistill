...
from bench_config import peak_efficiency_limits, require_blackwell_gpu  # noqa: E402

ref_folder = "../../performance_metrics/perf_G/golden_results"


def calculate(path_gen, path_ref):
    peak_gbps, peak_tflops = peak_efficiency_limits()
    ...
    efficiency = max(
        round(max(get_gbs(data_gen)) * 100 / peak_gbps, 4),
        round(max(get_tflops(data_gen)) * 100 / peak_tflops, 4),
    )
    efficiency1 = max(
        round(max(get_gbs(data_ref)) * 100 / peak_gbps, 4),
        round(max(get_tflops(data_ref)) * 100 / peak_tflops, 4),
    )
...
if __name__ == "__main__":
    ...
    peak_gbps, peak_tflops = peak_efficiency_limits()
    print(f"Blackwell efficiency limits: {peak_gbps} GB/s, {peak_tflops} TFLOPS ({gpu['name']})", flush=True)
    statis(args.gen_folder)
